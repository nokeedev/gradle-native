...
public class DocLinkBuilder {
	...
	public DocLink link(TypeMetaData type) {
		return link(type, DefaultGenerationListener.INSTANCE);
	}
	...
	public DocLink link(TypeMetaData type, final GenerationListener listener) {
		...
		if (linkElement.getChildNodes().getLength() == 1 && linkElement.getFirstChild() instanceof Element) {
			return new DocLinkImpl((Element) linkElement.getFirstChild());
		}
		return new DocLinkImpl(linkElement);
	}
	...
	public DocLink link(MethodMetaData method) {
		return link(method, DefaultGenerationListener.INSTANCE);
	}
	...
	@Value
	public static class DocLinkImpl implements DocLink {
		Element docbook;
	}
}
