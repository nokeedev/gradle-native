...
public abstract class DocumentationPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		SourceSet<Asciidoctor> dslAsciidoctor = sourceSetFactory.newSourceSet("dslAsciidoctor", Asciidoctor.class, assembleDslTask, assembleDslTask.flatMap(AssembleDslDocTask::getDestinationDirectory), "**/*.adoc");
		...
		TaskProvider<Sync> stageBakeTask = tasks.register("stageBake", Sync.class, task -> {
			...
			task.getInputs().files(dslAsciidoctor.getSource());
			task.from(dslAsciidoctor.getSource(), spec -> spec.into("content/docs/" + documentationVersion.get() + "/dsl"));
		});
