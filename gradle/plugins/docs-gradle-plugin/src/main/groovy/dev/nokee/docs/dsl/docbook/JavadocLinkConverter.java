...
public class JavadocLinkConverter {
	...
	public JavadocLinkConverter(Document document, TypeNameResolver typeNameResolver, DocLinkBuilder docLinkBuilder,
	...
	private Node doResolve(String link, ClassMetaData classMetaData, GenerationListener listener) {
		...
		if (matcher.group(2) == null) {
			return docLinkBuilder.link(new TypeMetaData(className), listener).getDocbook();
		}
		...
		if (targetClass.isEnum() && targetClass.getEnumConstant(methodSignature) != null) {
			return docLinkBuilder.link(targetClass.getEnumConstant(methodSignature), listener).getDocbook();
		}
		...
		return docLinkBuilder.link(method, listener).getDocbook();
	}
