...
final class AllGifs<T extends ExtensionAware> implements BiConsumer<T, CopySpec> {
	@Override
	public void accept(T sample, CopySpec spec) {
		spec.from(sourceDirectory(sample).map(it -> it.getAsFileTree().matching(t -> t.include("**/*.gif"))));
	}
