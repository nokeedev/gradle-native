...
@Nested
class XCConfigurationListCoderTests {
	@ParameterizedTest
	@EnumSource(CodeableXCConfigurationList.CodingKeys.class)
	void hasCodersForEachCodingKeys(CodingKey key) {
		MatcherAssert.assertThat(KeyedCoders.DECODERS.get(key), notNullValue());
	}

	@Test
	void hasBuildConfigurationsCoder() {
		assertThat(KeyedCoders.DECODERS.get(buildConfigurations), ofKey("buildConfigurations").asListOfRefObject(XCBuildConfigurationFactory.class));
	}

	@Test
	void hasDefaultConfigurationNameCoder() {
		assertThat(KeyedCoders.DECODERS.get(defaultConfigurationName), ofKey("defaultConfigurationName").asString());
	}

	@Test
	void hasDefaultConfigurationIsVisibleCoder() {
		assertThat(KeyedCoders.DECODERS.get(defaultConfigurationIsVisible), ofKey("defaultConfigurationIsVisible").asZeroOneBoolean());
	}
}
