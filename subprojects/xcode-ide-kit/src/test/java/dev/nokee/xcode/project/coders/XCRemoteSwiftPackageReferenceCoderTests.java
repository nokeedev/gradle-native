...
@Nested
class XCRemoteSwiftPackageReferenceCoderTests {
	@ParameterizedTest
	@EnumSource(CodeableXCRemoteSwiftPackageReference.CodingKeys.class)
	void hasCodersForEachCodingKeys(CodingKey key) {
		MatcherAssert.assertThat(KeyedCoders.DECODERS.get(key), notNullValue());
	}

	@Test
	void hasRepositoryUrlCoder() {
		assertThat(KeyedCoders.DECODERS.get(repositoryUrl), ofKey("repositoryURL").asString());
	}

	@Test
	void hasRequirementCoder() {
		assertThat(KeyedCoders.DECODERS.get(requirement), ofKey("requirement").asObject(VersionRequirementFactory.class));
	}
}
