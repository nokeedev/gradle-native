...
@Nested
class ProjectReferenceCoderTests {
	@ParameterizedTest
	@EnumSource(CodeableProjectReference.CodingKeys.class)
	void hasCodersForEachCodingKeys(CodingKey key) {
		MatcherAssert.assertThat(KeyedCoders.DECODERS.get(key), notNullValue());
	}

	@Test
	void hasProductGroupCoder() {
		assertThat(KeyedCoders.DECODERS.get(ProductGroup), ofKey("ProductGroup").asRefObject(PBXGroupFactory.class));
	}

	@Test
	void hasProjectRefCoder() {
		assertThat(KeyedCoders.DECODERS.get(ProjectRef), ofKey("ProjectRef").asRefObject(PBXFileReferenceFactory.class));
	}
}
