...
class CodeableVersionRequirementExactTests extends CodeableAdapterTester<CodeableVersionRequirementExact> implements VisitableTester<XCRemoteSwiftPackageReference.VersionRequirement.Visitor<?>> {
	...
	void checkGetBranch(String expectedValue) {
		val subject = newSubject(it -> it.when(callTo(method(KeyedObject_tryDecode())).with(args(version)).then(doReturn(expectedValue))));
	...
	@Test
	void checkToString() {
		val subject = newSubject(it -> it.when(callTo(method(KeyedObject_tryDecode())).with(args(version)).then(doReturn("4.2"))));
