...
class PBXProjReaderTest {
	...
	private static String pbxproj(BiFunction<String, GlobalIDs, String> generator) {
		return dict(archiveVersion(), classes(), objectVersion(), objectsWithRootObject(generator));
	}
	...
	@Test
	void canReadEmptyProject() {
		val subject = newReader(withUTF8Header(pbxproj(emptyProject())));
		val pbxproj = subject.read();
		assertThat(pbxproj.getObjects().getById(pbxproj.getRootObject()).isa(), equalTo("PBXProject"));
	}
}
