...
class ProjectReferenceDecoderTests {
	ValueDecoder.Context context = new ThrowingDecoderContext();
	ProjectReferenceDecoder<?> subject = new ProjectReferenceDecoder<>();

	@Nested
	class WhenIsaNull {
		KeyedObject map = new NullIsaKeyedObject();

		@Test
		void createsProjectReference() {
			assertThat(subject.decode(map, context), equalTo(CodeableProjectReference.newInstance(map)));
		}
	}
