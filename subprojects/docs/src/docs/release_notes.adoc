...
The Nokee plugins let you build JNI libraries <<sample/java-cpp-jni-library/index.adoc#,as a single project>> or by <<sample/jni-library-composing/index.adoc#,composing multiple projects together>>.
The native component of the JNI library can be implemented <<sample/java-c-jni-library/index.adoc#,in C>> or <<sample/java-cpp-jni-library/index.adoc#,in {cpp}>>.
More implementation language support will follow in future releases.
You can <<sample/jni-library-with-junit-test/index.adoc#,test your JNI library with JUnit as you would with any Java library>>.
Finally, <<sample/jni-library-with-target-machine/index.adoc#,the native component of the JNI library can target different machines>>.
...
Finally, publishing the JNI libraries to binary repositories requires customization of your build script, see <<sample/publish-jni-library/index.adoc#,this sample to learn more>>.
...
This release prepared the infrastructure for <<../nightly/manual/user_manual.adoc#,the documentation portal to provide automated nightly releases of the plugins>>.
It also rolled out a <<sample/index.adoc#,sample infrastructure to allow users to see how particular use cases are solved quickly>>.
