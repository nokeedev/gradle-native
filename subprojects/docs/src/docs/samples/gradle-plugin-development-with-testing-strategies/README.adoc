...
This sample shows how expand the testing coverage of a Gradle plugin to multiple Gradle distribution.
...
=====
====
<1> The Nokee team maintain the Gradle fixtures which cover common testing scenarios and are testing strategy aware.
The Gradle executer will honor the selected testing strategy for the `Test` task.
...
Gradle executes the test suites for all testing coverage, that is minimum supported Gradle, latest released Gradle, and latest nightly.
