...
In this case, Gradle is the single source of truth, and it converts only the required information into an Xcode-compatible model.
...
For this reason, the plugin registers an extension on the root Gradle project to provide the additional capability for generating a workspace.
