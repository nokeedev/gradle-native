...
We can execute the test suite via the link:{gradle-dsl-reference}/org.gradle.api.tasks.testing.Test.html[`Test`] task named `test`.
...
We can execute the test suite via the link:{gradle-dsl-reference}/org.gradle.api.tasks.testing.Test.html[`Test`] task named `functionalTest`.
