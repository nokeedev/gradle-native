...
An JVM (i.e. the link:{gradle-user-manual}/java_plugin.html[Java Plugin]) and native implementation (i.e. the <<cpp_language_plugin.adoc#,{cpp} Language Plugin>>) language plugins are required to be applied to produce anything meaningful.
...
`jniJar__Variant__` (e.g. `jarWindows` and `jarLinux`) - link:{gradle-language-reference}/org.gradle.api.tasks.bundling.Jar.html[Jar]::
Depends on: `sharedLibrary__Variant__`
::
Assembles the production JAR file containing the shared library for the JNI bridge as well as all the runtime dependencies for the particular variant.
