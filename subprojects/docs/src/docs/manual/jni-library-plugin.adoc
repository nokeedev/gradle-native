...
It doesn't provide any implementation language capabilities and thus needs to be composed by applying other implementation language plugins, such as <<cpp-language-plugin.adoc#,{cpp} Language Plugin>>, <<c-language-plugin.adoc#,C Language Plugin>>, <<objective-c-language-plugin.adoc#,Objective-C Language Plugin>> and link:{gradle-user-manual}/java_plugin.html[Java Plugin].
