...
This release focus on adding <<release-notes.adoc#sec:macos-framework,support for macOS framework>>, adding <<release-notes.adoc#sec:objc-objcpp,support for Objective-C and Objective-C++ implementation languages>>, extend <<release-notes.adoc#sec:testing-coverage,automated test coverage to macOS, Linux, and Windows>>, and adding <<release-notes.adoc#sec:jni-resource-path,configuration for the resource path for the native libraries embedded inside the Java Native Interface (JNI) library>>.
...
The link:samples/jni-library-with-framework-dependencies[macOS framework dependencies are supported as normal dependencies].
...
Instead, the frameworks are grouped on the command line passed to the compiler and linker.
...
Finally, the dynamic version resolution of frameworks is not supported in this release.
...
The JNI library plugin now support link:samples/java-objective-c-jni-library[Objective-C] and link:samples/java-objective-cpp-jni-library[Objective-{cpp}] implementation language via <<manual/plugin-references.adoc#sec:native-language-plugins,language plugins>>.
...
Native toolchains coverage is still partial and will be improved in the next release.
