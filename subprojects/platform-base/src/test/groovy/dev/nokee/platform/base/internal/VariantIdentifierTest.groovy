...
class VariantIdentifierTest extends Specification {
	...
	def "computes display name from unambiguous name and component's display name for non-main component"() {
		...
		def testOwnerIdentifier = ComponentIdentifier.of('test', ProjectIdentifier.of('root'))
		def integTestOwnerIdentifier = ComponentIdentifier.of('integTest', ProjectIdentifier.of('root'))

		expect:
		VariantIdentifier.of('macosDebug', mainOwnerIdentifier).displayName == "variant 'macosDebug'"
		VariantIdentifier.of('macosDebug', testOwnerIdentifier).displayName == "variant 'macosDebug' of component 'test'"
		VariantIdentifier.of('macosDebug', integTestOwnerIdentifier).displayName == "variant 'macosDebug' of component 'integTest'"
		VariantIdentifier.of('debug', mainOwnerIdentifier).displayName == "variant 'debug'"
	}

	def "includes component custom display name for main component"() {
		given:
		def jniOwnerIdentifier = ComponentIdentifier.builder().withName('main').withProjectIdentifier(ProjectIdentifier.of('root')).withDisplayName('JNI library').build()
		def iosOwnerIdentifier = ComponentIdentifier.builder().withName('main').withProjectIdentifier(ProjectIdentifier.of('root')).withDisplayName('iOS application').build()

		expect:
		VariantIdentifier.of('macosDebug', jniOwnerIdentifier).displayName == "variant 'macosDebug' of JNI library"
		VariantIdentifier.of('debug', iosOwnerIdentifier).displayName == "variant 'debug' of iOS application"
	}
	...
	def "uses component display name for empty unambiguous name"() {
		...
		VariantIdentifier.of('', mainOwnerIdentifier).displayName == "main component"
		VariantIdentifier.of('', testOwnerIdentifier).displayName == "component 'test'"
		VariantIdentifier.of('', jniOwnerIdentifier).displayName == "JNI library"
		VariantIdentifier.of('', jniOwnerIdentifier).displayName == "iOS application"
	}
}
