...
class DescribeDependencyBucketTest extends Specification {
	@Unroll
	def "can generate name for single word bucket name owned by a project"(projectName) {
		given:
		def identifier = DependencyIdentifier.of(DependencyBucketName.of('implementation'), TestableBucket, ProjectIdentifier.of(projectName))
	...
	def "can generate name for two words bucket name owned by a project"(bucketName, expectedValue) {
		given:
		def identifier = DependencyIdentifier.of(DependencyBucketName.of(bucketName), TestableBucket, ProjectIdentifier.of('root'))
	...
	def "can generate name for single word bucket name owned by the main component"() {
		given:
		def identifier = DependencyIdentifier.of(DependencyBucketName.of('implementation'), TestableBucket, ComponentIdentifier.ofMain(ProjectIdentifier.of('root')))
	...
	def "can generate name for single word bucket name owned by a non-main component"(componentName) {
		given:
		def identifier = DependencyIdentifier.of(DependencyBucketName.of('implementation'), TestableBucket, ComponentIdentifier.of(componentName, ProjectIdentifier.of('root')))
	...
	def "can generate name for single word bucket name owned by a variant of the main component"(unambiguousVariantName) {
		given:
		def identifier = DependencyIdentifier.of(DependencyBucketName.of('implementation'), TestableBucket, VariantIdentifier.of(unambiguousVariantName, ComponentIdentifier.ofMain(ProjectIdentifier.of('root'))))
	...
	def "can generate name for single word bucket name owned by a variant of the non-main component"(unambiguousVariantName, componentName) {
		given:
		def identifier = DependencyIdentifier.of(DependencyBucketName.of('implementation'), TestableBucket, VariantIdentifier.of(unambiguousVariantName, ComponentIdentifier.of(componentName, ProjectIdentifier.of('root'))))
	...
	def "uses component display name for single variant"() {
		given:
		def mainOwnerIdentifier = VariantIdentifier.of('', ComponentIdentifier.ofMain(ProjectIdentifier.of('root')))
		def testOwnerIdentifier = VariantIdentifier.of('', ComponentIdentifier.of('test', ProjectIdentifier.of('root')))
		def customDisplayNameOwnerIdentifier = VariantIdentifier.of('', ComponentIdentifier.builder().withName('test').withProjectIdentifier(ProjectIdentifier.of('root')).withDisplayName('JNI library').build())
	...
	private static <T extends DependencyBucket> DependencyIdentifier<T> identifier(String name, Class<T> type = TestableBucket, DomainObjectIdentifierInternal owner = ProjectIdentifier.of('root')) {
		return DependencyIdentifier.of(DependencyBucketName.of(name), type, owner)
	}
