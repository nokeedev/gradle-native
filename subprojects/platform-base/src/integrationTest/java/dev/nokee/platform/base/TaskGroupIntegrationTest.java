...
class TaskGroupIntegrationTest {
	...
	void createSubject(Project project) {
		subject = project.getExtensions().getByType(ModelRegistry.class).instantiate(builder()
			...
			.withComponent(new TaskProjectionComponent(task = project.getTasks().register("test")))
			.build());
	}

	@Test
	void configuresTaskProjectionGroupWithTaskGroupComponent() {
		subject.addComponent(new TaskGroupComponent("build"));
		assertThat(task.get(), TaskMatchers.group("build"));
	}
}
