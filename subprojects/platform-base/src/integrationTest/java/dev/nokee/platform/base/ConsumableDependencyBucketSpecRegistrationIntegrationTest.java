...
class ConsumableDependencyBucketSpecRegistrationIntegrationTest {
	@GradleProject Project project;
	ModelNode subject;

	@BeforeEach
	void setup() {
		subject = project.getExtensions().getByType(ModelRegistry.class).instantiate(newEntity("lini", ConsumableDependencyBucketSpec.class));
	}

	@Test
	void hasConsumableDependencyBucketSpecTag() {
		assertTrue(subject.hasComponent(typeOf(ConsumableDependencyBucketSpec.Tag.class)));
	}
	...
	void hasElementNameComponent() {
		assertThat(subject.find(ElementNameComponent.class).map(ElementNameComponent::get),
			optionalWithValue(equalTo(ElementName.of("lini"))));
	}
