...
class ResolvableDependencyBucketSpecRegistrationIntegrationTest {
	@GradleProject Project project;
	ModelNode subject;

	@BeforeEach
	void setup() {
		subject = project.getExtensions().getByType(ModelRegistry.class).instantiate(newEntity("dipu", ResolvableDependencyBucketSpec.class));
	}

	@Test
	void hasResolvableDependencyBucketSpecTag() {
		assertTrue(subject.hasComponent(typeOf(ResolvableDependencyBucketSpec.Tag.class)));
	}
	...
	void hasElementNameComponent() {
		assertThat(subject.find(ElementNameComponent.class).map(ElementNameComponent::get),
			optionalWithValue(equalTo(ElementName.of("dipu"))));
	}
