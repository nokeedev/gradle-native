...
class NamedDomainObjectProviderDelegateTest extends MethodCallForwardingTester {
	private static final NamedDomainObjectProvider<?> subject = new NamedDomainObjectProviderFactory().create(of(ProviderTestUtils.newNamedDomainObjectProviderProxy(handler)));
	...
	public Stream<Method> allMethodsUnderTest() {
		return ProviderTestUtils.allNamedDomainObjectProviderInterfaces().flatMap(it -> Arrays.stream(it.getMethods()));
	}
}
