...
public class CApplicationPlugin implements Plugin<Project> {
	...
	public static ModelRegistration cApplication(String name, Project project) {
		val identifier = ComponentIdentifier.builder().name(ElementName.of(name)).displayName("C application").withProjectIdentifier(ProjectIdentifier.of(project)).build();
