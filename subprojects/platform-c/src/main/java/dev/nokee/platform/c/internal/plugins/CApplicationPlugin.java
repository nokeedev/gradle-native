...
public class CApplicationPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		model(project, factoryRegistryOf(Component.class)).registerFactory(DefaultCApplication.class, new ModelObjectFactory<DefaultCApplication>(project, IsComponent.class) {
			@Override
			protected DefaultCApplication doCreate(String name) {
				return project.getObjects().newInstance(DefaultCApplication.class, model(project, registryOf(DependencyBucket.class)), model(project, registryOf(Task.class)), project.getExtensions().getByType(new TypeOf<Factory<BinaryView<Binary>>>() {}), project.getExtensions().getByType(new TypeOf<Factory<SourceView<LanguageSourceSet>>>() {}));
			}
		});
		...
		, CSourcesMixIn
	{
		...
		@Inject
		public DefaultCApplication(ModelObjectRegistry<DependencyBucket> bucketRegistry, ModelObjectRegistry<Task> taskRegistry, Factory<BinaryView<Binary>> binariesFactory, Factory<SourceView<LanguageSourceSet>> sourcesFactory) {
