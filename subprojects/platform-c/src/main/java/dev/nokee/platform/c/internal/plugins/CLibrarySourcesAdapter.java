...
public final class CLibrarySourcesAdapter implements CLibrarySources {
	private final ViewAdapter<LanguageSourceSet> delegate;

	public CLibrarySourcesAdapter(ViewAdapter<LanguageSourceSet> delegate) {
    ...
    @Override
    public void c(Action<? super CSourceSet> action) {
        delegate.named("c", CSourceSet.class, action);
	}

    @Override
    public void c(@SuppressWarnings("rawtypes") Closure closure) {
		delegate.named("c", CSourceSet.class, closure);
	}
