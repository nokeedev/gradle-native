...
public class CLibraryPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		model(project, factoryRegistryOf(Component.class)).registerFactory(DefaultCLibrary.class, new ModelObjectFactory<DefaultCLibrary>(project, IsComponent.class) {
			...
			protected DefaultCLibrary doCreate(String name) {
				return project.getObjects().newInstance(DefaultCLibrary.class, model(project, registryOf(DependencyBucket.class)), model(project, registryOf(Task.class)));
			}
		});
		...
		, CSourcesMixIn
	{
		...
		@Inject
		public DefaultCLibrary(ModelObjectRegistry<DependencyBucket> bucketRegistry, ModelObjectRegistry<Task> taskRegistry) {
