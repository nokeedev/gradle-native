...
public class CLibraryPlugin implements Plugin<Project> {
	...
	public static ModelRegistration cLibrary(String name, Project project) {
		...
		return new NativeLibraryComponentModelRegistrationFactory(CLibrary.class, DefaultCLibrary.class, project, (entity, path) -> {
			...
			registry.register(project.getExtensions().getByType(ComponentSourcesPropertyRegistrationFactory.class).create(ModelPropertyIdentifier.of(identifier, "sources"), CLibrarySources.class, CLibrarySourcesAdapter::new));
		}).create(identifier);
	}
