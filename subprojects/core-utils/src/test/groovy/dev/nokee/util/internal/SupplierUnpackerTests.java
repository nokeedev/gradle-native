...
class SupplierUnpackerTests {
	@Nested
	class WhenUnpackingSupplier {
		Unpacker delegate = newAlwaysThrowingMock(Unpacker.class);
		SupplierUnpacker subject = new SupplierUnpacker(delegate);
		Object result = subject.unpack(supplierOf("supplied-foo"));

		@Test
		void returnSupplierValue() {
			assertThat(result, equalTo("supplied-foo"));
		}

		@Test
		void canUnpack() {
			assertThat(subject.canUnpack(supplierOf(throwIfResolved())), equalTo(true));
		}
	}

	@Test
	void canUnpackNullReturningSupplier() {
		assertThat(new SupplierUnpacker(newAlwaysThrowingMock(Unpacker.class)).unpack(supplierOf(null)), nullValue());
	}

	@Nested
	class ForwardAnyValueToDelegate {
		ForwardingWrapper<Unpacker> subject = ForwardingWrapper.forwarding(Unpacker.class, SupplierUnpacker::new);
