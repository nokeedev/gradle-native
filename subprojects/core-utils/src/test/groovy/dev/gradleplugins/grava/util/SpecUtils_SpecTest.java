...
import static dev.gradleplugins.grava.util.SpecUtils.Spec.of;
...
class SpecUtils_SpecTest {
	...
	void canWrapGradleSpec() {
		assertThat(of(it -> true), isA(dev.gradleplugins.grava.util.SpecUtils.Spec.class));
	}
	...
	class FluentOr implements SpecOrTester {
		@Override
		public <T> Spec<T> createOrSpec(dev.gradleplugins.grava.util.SpecUtils.Spec<T> first, Spec<? super T> second) {
	...
	class FluentNegate implements SpecNegateTester {
		@Override
		public <T> dev.gradleplugins.grava.util.SpecUtils.Spec<T> createNegateSpec(dev.gradleplugins.grava.util.SpecUtils.Spec<T> spec) {
			return spec.negate();
		}
	}
