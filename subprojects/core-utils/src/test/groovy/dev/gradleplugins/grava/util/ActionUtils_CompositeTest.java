...
class ActionUtils_CompositeTest {
	...
	void returnsSpecifiedActionWhenComposingOnlyOneAction() {
		dev.gradleplugins.grava.util.ActionUtils.Action<String> action = t -> {};
	...
	void returnsEnhanceAction() {
		...
		assertThat(composite(action, action), isA(dev.gradleplugins.grava.util.ActionUtils.Action.class));
	}
