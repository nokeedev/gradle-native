...
public final class CompositeUnpacker implements Unpacker {
	private final Unpacker first;
	private final Unpacker second;

	public CompositeUnpacker(Unpacker first, Unpacker second) {
		this.first = first;
		this.second = second;
	}
	...
	public Object unpack(@Nullable Object target) {
		return second.unpack(first.unpack(target));
	}

	@Override
	public boolean canUnpack(@Nullable Object target) {
		return first.canUnpack(target) || second.canUnpack(target);
	}
}
