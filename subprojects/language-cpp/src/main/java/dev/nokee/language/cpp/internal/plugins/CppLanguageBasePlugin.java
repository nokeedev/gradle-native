	...
	public void apply(Project project) {
		...
		model(project, factoryRegistryOf(LanguageSourceSet.class)).registerFactory(CppSourceSetSpec.class, new ModelObjectFactory<CppSourceSetSpec>(project, IsLanguageSourceSet.class) {
			...
			protected CppSourceSetSpec doCreate(String name) {
				return project.getObjects().newInstance(CppSourceSetSpec.class, model(project, registryOf(DependencyBucket.class)));
			}
		});
		...
		// ComponentFromEntity<GradlePropertyComponent> read-write on CppSourcesPropertyComponent
		project.getExtensions().getByType(ModelConfigurer.class).configure(ModelActionWithInputs.of(ModelComponentReference.of(CppSourcesPropertyComponent.class), ModelComponentReference.of(ExtensionAwareComponent.class), (entity, cppSources, extensions) -> {
