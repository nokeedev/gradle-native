...
class DefaultModelNodeTest {
	...
	void doesNotRealizeProjectionProvider() {
		val provider = rootProject().getObjects().domainObjectContainer(TestObject.class).register("foo", it -> new RuntimeException(""));
		val projection = subject.newProjection(builder -> builder.type(TestProjection.class).forProvider(provider));
		assertAll(
			() -> assertThat(projection, notNullValue()),
			() -> assertThat(projection.canBeViewedAs(TestProjection.class), is(true)),
			() -> assertThat(projection.canBeViewedAs(UnknownProjection.class), is(false)),
	...
	public static class TestObject implements Named {
