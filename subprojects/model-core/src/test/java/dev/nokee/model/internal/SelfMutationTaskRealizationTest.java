...
class SelfMutationTaskRealizationTest {
	private final Project project = rootProject();
	private final Action<Task> a1 = Mockito.mock(Action.class);
	private final Action<Task> a2 = Mockito.mock(Action.class);
	private final Action<Task> a3 = Mockito.mock(Action.class);
	private final InOrder inOrder = Mockito.inOrder(a1, a2, a3);
	...
	void setup() {
		...
		mainProjection.whenRealized(a -> {
			compileProjection.whenRealized(a3);
		});
		compileProjection.whenRealized(a1);
		compileProjection.whenRealized(a2);

		Mockito.verify(a1, never()).execute(any());
		Mockito.verify(a2, never()).execute(any());
		Mockito.verify(a3, never()).execute(any());
	}

	@Test
	void executesSelfMutatingActionAfterAlreadyRegisteredActionWhenProjectionRealized() {
		compileProjection.realize();
		inOrder.verify(a1).execute(isA(Task.class));
		inOrder.verify(a2).execute(isA(Task.class));
		inOrder.verify(a3).execute(isA(Task.class));
	}

	@Test
	void executesSelfMutatingActionAfterAlreadyRegisteredActionWhenProviderRealized() {
