...
public class CppLibraryPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		components(project).registerFactory(DefaultCppLibrary.class, new ModelObjectFactory<DefaultCppLibrary>(project, IsComponent.class) {
			@Override
			protected DefaultCppLibrary doCreate(String name) {
				return project.getObjects().newInstance(DefaultCppLibrary.class);
			}
		});
	...
	public static ModelRegistration cppLibrary(String name, Project project) {
		val identifier = ComponentIdentifier.builder().name(ElementName.of(name)).displayName("C++ library").withProjectIdentifier(ProjectIdentifier.of(project)).build();
