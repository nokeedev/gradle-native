...
public class CppLibraryPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		model(project, factoryRegistryOf(Component.class)).registerFactory(DefaultCppLibrary.class, new ModelObjectFactory<DefaultCppLibrary>(project, IsComponent.class) {
			...
			protected DefaultCppLibrary doCreate(String name) {
				return project.getObjects().newInstance(DefaultCppLibrary.class, model(project, registryOf(DependencyBucket.class)), model(project, registryOf(Task.class)), project.getExtensions().getByType(new TypeOf<Factory<BinaryView<Binary>>>() {}), project.getExtensions().getByType(new TypeOf<Factory<SourceView<LanguageSourceSet>>>() {}));
			}
		});
		...
		, CppSourcesMixIn
	{
		...
		@Inject
		public DefaultCppLibrary(ModelObjectRegistry<DependencyBucket> bucketRegistry, ModelObjectRegistry<Task> taskRegistry, Factory<BinaryView<Binary>> binariesFactory, Factory<SourceView<LanguageSourceSet>> sourcesFactory) {
