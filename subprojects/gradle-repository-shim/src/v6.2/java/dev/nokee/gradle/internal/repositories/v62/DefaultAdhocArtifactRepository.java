 ...
 */
package dev.nokee.gradle.internal.repositories.v62;
...
// Note: We have to extend from AbstractArtifactRepository because ResolveIvyFactory conditionally check if the repository is an AbstractArtifactRepository and use AbstractArtifactRepository#getContentFilter() which was how ContentFilteringRepository was implemented in previous Gradle versions, including this one.
//   However, we gut the implementation so it looks like the newer versions.
public final class DefaultAdhocArtifactRepository extends AbstractArtifactRepository implements AdhocArtifactRepository, ResolutionAwareRepository {
	...
	public DefaultAdhocArtifactRepository(MavenArtifactRepository delegate, ObjectFactory objects) {
		super(objects); // just to make the parent implementation happy
	...
	public Action<? super ArtifactResolutionDetails> getContentFilter() {
		return ((AbstractArtifactRepository) delegate).getContentFilter();
	}
