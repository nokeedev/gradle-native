...
public class ObjectiveCApplicationPlugin implements Plugin<Project> {
	...
	public static ModelRegistration objectiveCApplication(String name, Project project) {
		val identifier = ModelObjectIdentifier.builder().name(ElementName.of(name)).displayName("Objective-C application").withParent(ProjectIdentifier.of(project)).build();
