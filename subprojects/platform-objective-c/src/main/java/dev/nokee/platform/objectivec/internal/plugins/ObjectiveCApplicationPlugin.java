...
public class ObjectiveCApplicationPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		components(project).registerFactory(DefaultObjectiveCApplication.class, new ModelObjectFactory<DefaultObjectiveCApplication>(project, IsComponent.class) {
			@Override
			protected DefaultObjectiveCApplication doCreate(String name) {
				return project.getObjects().newInstance(DefaultObjectiveCApplication.class);
			}
		});
	...
	public static ModelRegistration objectiveCApplication(String name, Project project) {
		val identifier = ComponentIdentifier.builder().name(ElementName.of(name)).displayName("Objective-C application").withProjectIdentifier(ProjectIdentifier.of(project)).build();
