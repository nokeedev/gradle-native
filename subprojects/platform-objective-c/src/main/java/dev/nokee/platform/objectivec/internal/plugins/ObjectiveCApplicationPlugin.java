...
public class ObjectiveCApplicationPlugin implements Plugin<Project> {
	...
	public static ModelRegistration objectiveCApplication(String name, Project project) {
		...
		return new NativeApplicationComponentModelRegistrationFactory(DefaultObjectiveCApplication.class, project).create(identifier).withComponent(ObjectiveCSourceSetTag.tag()).build();
	}
