...
import dev.nokee.language.objectivec.internal.plugins.ObjectiveCSourceSetComponent;
...
class ObjectiveCSourceSetAwareComponentIntegrationTest extends AbstractPluginTest {
	...
	void createSubject() {
		...
		assertThat("expect no source set", componentEntity.has(ObjectiveCSourceSetComponent.class), is(false));
		componentEntity.addComponent(NativeLanguageSourceSetAwareTag.tag());
		sourceSetEntity = componentEntity.find(ObjectiveCSourceSetComponent.class).map(LinkedEntity::get).orElse(null);
	}

	@Test
	void hasSourceSetComponent() {
		assertThat(componentEntity.has(ObjectiveCSourceSetComponent.class), is(true));
	}

	@Test
	void usesDefaultSourceSetName() {
		assertThat(sourceSetEntity.get(ElementNameComponent.class).get(), equalTo(ElementName.of("objectiveC")));
	}
