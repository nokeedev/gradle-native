...
class ObjectiveCSourceSetBuildableIntegrationTest extends AbstractPluginTest {
	private ObjectiveCSourceSetSpec subject;

	@BeforeEach
	void createSubject() {
		project.getPluginManager().apply(NokeeStandardToolChainsPlugin.class);
		subject = project.getExtensions().getByType(ModelRegistry.class).register(project.getExtensions().getByType(ObjectiveCSourceSetRegistrationFactory.class).create(LanguageSourceSetIdentifier.of(ProjectIdentifier.of(project), "duqo"))).as(ObjectiveCSourceSetSpec.class).get();
	}
