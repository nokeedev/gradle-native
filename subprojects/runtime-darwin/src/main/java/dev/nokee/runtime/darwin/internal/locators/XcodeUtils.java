...
public class XcodeUtils {
	...
	public static File findTool(String name) {
		return CommandLineTool.fromPath("xcrun").get().withArguments("--find", name).execute(new ProcessBuilderEngine()).waitFor().assertNormalExitValue().getStandardOutput().parse(asPath());
	}
