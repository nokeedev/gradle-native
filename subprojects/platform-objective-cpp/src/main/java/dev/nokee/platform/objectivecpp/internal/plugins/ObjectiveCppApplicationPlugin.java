...
public class ObjectiveCppApplicationPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		model(project, factoryRegistryOf(Component.class)).registerFactory(DefaultObjectiveCppApplication.class, new ModelObjectFactory<DefaultObjectiveCppApplication>(project, IsComponent.class) {
			...
			protected DefaultObjectiveCppApplication doCreate(String name) {
				return project.getObjects().newInstance(DefaultObjectiveCppApplication.class, model(project, registryOf(DependencyBucket.class)), model(project, registryOf(Task.class)));
			}
		});
		...
		, PrivateHeadersMixIn
	{
		...
		@Inject
		public DefaultObjectiveCppApplication(ModelObjectRegistry<DependencyBucket> bucketRegistry, ModelObjectRegistry<Task> taskRegistry) {
