...
public class ObjectiveCIosApplicationPlugin implements Plugin<Project> {
	...
	public static ModelRegistration objectiveCIosApplication(String name, Project project) {
		val identifier = ComponentIdentifier.builder().name(ElementName.of(name)).displayName("Objective-C iOS application").withProjectIdentifier(ProjectIdentifier.of(project)).build();
