...
public class ObjectiveCIosApplicationPlugin implements Plugin<Project> {
	...
	public static ModelRegistration objectiveCIosApplication(String name, Project project) {
		val identifier = ModelObjectIdentifier.builder().name(ElementName.of(name)).displayName("Objective-C iOS application").withParent(ProjectIdentifier.of(project)).build();
