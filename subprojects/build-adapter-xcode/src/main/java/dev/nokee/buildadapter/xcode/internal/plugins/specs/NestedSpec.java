...
public final class NestedSpec implements XCBuildSpec {
	...
	@Nested
	public Map<String, XCBuildSpec> getValue() {
		return values;
	}

	@Override
	public XCBuildSpec resolve(ResolveContext context) {
		return new NestedSpec(values.entrySet().stream().map(it -> new AbstractMap.SimpleImmutableEntry<>(it.getKey(), it.getValue().resolve(context))).collect(ImmutableMap.toImmutableMap(Map.Entry::getKey, Map.Entry::getValue)));
	}
