...
//		put(CodeableXCConfigurationList.CodingKeys.buildConfigurations, forKey("buildConfigurations", listOf(object(XCBuildConfiguration.class))));
	...
	private final Set<CodingKey> encodableKeys = ImmutableSet.<CodingKey>builder()
		// TODO: files are input to this build phase and should be unmixed to ignore intermediate files
		//    Do the unmixing during resolution
//		.add(CodeablePBXNativeTarget.CodingKeys.buildPhases)
//		.add(CodeablePBXLegacyTarget.CodingKeys.buildPhases)
//		.add(CodeablePBXAggregateTarget.CodingKeys.buildPhases)

		// TODO: should serialize as output file
//		.add(CodeablePBXNativeTarget.CodingKeys.productReference)
//		.add(CodeablePBXLegacyTarget.CodingKeys.productReference)
//		.add(CodeablePBXAggregateTarget.CodingKeys.productReference)

		// TODO: Should serialize as input file but should also unmix them
//		.add(CodeablePBXFrameworksBuildPhase.CodingKeys.files)
//		.add(CodeablePBXCopyFilesBuildPhase.CodingKeys.files)
//		.add(CodeablePBXHeadersBuildPhase.CodingKeys.files)
//		.add(CodeablePBXResourcesBuildPhase.CodingKeys.files)
//		.add(CodeablePBXShellScriptBuildPhase.CodingKeys.files)
//		.add(CodeablePBXSourcesBuildPhase.CodingKeys.files)

//		.add(CodeablePBXBuildFile.CodingKeys.productRef)
		.build();
	...
	private static <T extends Codeable, U /*extends PBXObject*/> ValueEncoder<XCBuildSpec, T> object(Class<U> type) {
		return new NestedObjectSpecEncoder<>(new NoOpEncoder<>(CoderType.of(Object.class)));
	}
	...
	private static ValueEncoder<XCBuildSpec, Object> inputLocation() {
		return new InputLocationSpecEncoder<>(new ResolvablePathEncoder(new NormalizeStringAsPBXReferenceEncoder(new NormalizePBXBuildFileFileReferenceAsPBXReferenceEncoder())));
	}

	private static ValueEncoder<XCBuildSpec, Object> outputLocation() {
		return new OutputLocationSpecEncoder<>(new ResolvablePathEncoder(new NormalizeStringAsPBXReferenceEncoder(new NormalizePBXBuildFileFileReferenceAsPBXReferenceEncoder())));
	}

	private static <T, OUT> ValueEncoder<XCBuildSpec, List<T>> listOf(ValueEncoder<XCBuildSpec, T> elementEncoder) {
	...
	private static final class EncoderOnlyCoder<T> implements ValueCoder<T> {
		...
		@Override
		public CoderType<? super T> getDecodeType() {
			throw new UnsupportedOperationException();
		}
