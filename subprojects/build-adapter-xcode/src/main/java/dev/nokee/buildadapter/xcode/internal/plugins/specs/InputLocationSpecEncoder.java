...
public final class InputLocationSpecEncoder<IN> implements ValueEncoder<XCBuildSpec, IN> {
	...
	public InputLocationSpecEncoder(ValueEncoder<ResolvablePathEncoder.LocationSpec, IN> delegate) {
	...
	public CoderType<?> getEncodeType() {
		return inputLocation(delegate.getEncodeType());
	}

	public static CoderType<XCBuildSpec> inputLocation(CoderType<?> type) {
		return new CoderInputLocationType(type);
	}
}
