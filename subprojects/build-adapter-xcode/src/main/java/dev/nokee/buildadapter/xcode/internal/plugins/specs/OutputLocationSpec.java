...
@EqualsAndHashCode
public final class OutputLocationSpec implements XCBuildSpec {
	private final ResolvablePathEncoder.LocationSpec delegate;

	OutputLocationSpec(ResolvablePathEncoder.LocationSpec delegate) {
	...
	public XCBuildSpec resolve(ResolveContext context) {
		return new OutputLocationSpec(delegate.resolve(new ResolvablePathEncoder.LocationSpec.ResolveContext() {
			@Override
			public FileCollection resolve(PBXReference reference) {
				return context.outputs(reference); // resolve as outputs
			}
		}));
	}
