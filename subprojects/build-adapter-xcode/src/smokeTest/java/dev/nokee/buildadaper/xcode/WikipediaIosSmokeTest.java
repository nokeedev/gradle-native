...
class WikipediaIosSmokeTest {
	...
	static void setup(GradleRunner runner) throws IOException {
		CommandLine.of("git", "clone", "--depth=1", "https://github.com/wikimedia/wikipedia-ios.git", '.').execute(null, testDirectory).waitFor().assertNormalExitValue();
		plugins(it -> it.id("dev.nokee.xcode-build-adapter")).writeTo(testDirectory.resolve("settings.gradle"));
		executer = runner.withTasks("WikipediaDebug").withArgument("-Dconfiguration=Debug").withArgument("-Dsdk=iphonesimulator");
