...
class XCTargetTaskGroupRuleTests {
	ModelAction subject = new XCTargetTaskGroupRule();
	ModelNode target;
	ModelNode task;
	...
	@Test
	void addsXcodeTargetTaskGroupToTargetTask() {
		subject.execute(target);

		assertThat(task.get(TaskGroupComponent.class).get(), equalTo("Xcode Target"));
	}
}
