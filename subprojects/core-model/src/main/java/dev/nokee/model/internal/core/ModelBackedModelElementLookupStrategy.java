...
public final class ModelBackedModelElementLookupStrategy implements ModelElementLookupStrategy {
	...
	public ModelElement get(String name) {
		assert name != null;
		return elementFactory.createElement(ModelNodeUtils.getDirectDescendants(entity).stream().filter(it -> it.get(ElementNameComponent.class).get().toString().equals(name)).collect(MoreCollectors.onlyElement()));
	}
	...
	public <S> DomainObjectProvider<S> get(String name, ModelType<S> type) {
		assert name != null;
		assert type != null;
		return elementFactory.createObject(ModelNodeUtils.getDirectDescendants(entity).stream().filter(it -> it.get(ElementNameComponent.class).get().toString().equals(name) && ModelNodeUtils.canBeViewedAs(it, type)).collect(MoreCollectors.onlyElement()), type);
	}
}
