...
class ModelPropertyListPropertyIntegrationTest {
	...
	void setUp() {
		...
		subject = project.getExtensions().getByType(ModelRegistry.class).instantiate(builder()
			...
			.withComponent(new ModelPropertyTypeComponent(list(of(MyType.class))))
			.build());
	}
	...
	void usesConfigurableFileCollectionAsGradleProperty() {
		assertTrue(subject.get(GradlePropertyComponent.class).get() instanceof ListProperty);
	}
