...
public abstract class VisualStudioIdeBridge extends IdeBridgeRule<VisualStudioIdeRequest> {
	...
	private DefaultVisualStudioIdeTarget findVisualStudioTarget(VisualStudioIdeRequest request) {
		...
		DefaultVisualStudioIdeTarget target = project.getTargets().stream().filter(DefaultVisualStudioIdeTarget.class::isInstance).map(DefaultVisualStudioIdeTarget.class::cast).filter(it -> it.getProjectConfiguration().equals(projectConfiguration)).findFirst().orElse(null);
		if (target == null) {
