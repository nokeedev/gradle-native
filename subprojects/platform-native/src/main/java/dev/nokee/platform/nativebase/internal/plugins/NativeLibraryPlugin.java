...
public class NativeLibraryPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		project.getExtensions().getByType(ModelLookup.class).get(ModelPath.path("components")).getComponent(ModelComponentType.componentOf(NodeRegistrationFactories.class)).registerFactory(ModelType.of(NativeLibrarySpec.class), name -> nativeLibrary(name, project));
		val components = project.getExtensions().getByType(ComponentContainer.class);
		val componentElement = components.register("main", NativeLibrarySpec.class);
		val componentProvider = componentElement.as(NativeLibraryExtension.class);
	...
	public interface NativeLibrarySpec extends ComponentSpec {}
	...
	public static abstract class DefaultNativeLibraryExtension implements NativeLibraryExtension, ModelBackedDependencyAwareComponentMixIn<NativeLibraryComponentDependencies>, ModelBackedSourceAwareComponentMixIn<NativeLibrarySources>, ModelBackedVariantAwareComponentMixIn<NativeLibrary>, ModelBackedBinaryAwareComponentMixIn {
	}
	...
	public static NodeRegistration nativeLibraryVariant(VariantIdentifier<DefaultNativeLibraryVariant> identifier, DefaultNativeLibraryComponent component, Project project) {
		...
		return NodeRegistration.unmanaged(identifier.getUnambiguousName(), of(NativeLibrary.class), () -> {
			...
			val result = project.getObjects().newInstance(DefaultNativeLibraryVariant.class, identifier, variantDependencies.getIncoming(), project.getObjects(), project.getProviders(), assembleTask, project.getExtensions().getByType(BinaryViewFactory.class));
