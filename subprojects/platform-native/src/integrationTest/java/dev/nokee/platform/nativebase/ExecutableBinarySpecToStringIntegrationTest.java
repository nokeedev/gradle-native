...
class ExecutableBinarySpecToStringIntegrationTest {
	...
	void createSubject() {
		...
		subject = registry.register(factory.create(projectIdentifier.child("robe"))).as(ExecutableBinary.class).get();
	}

	@Test
	void checkToString() {
		assertThat(subject, hasToString("executable binary 'robe'"));
	}
}
