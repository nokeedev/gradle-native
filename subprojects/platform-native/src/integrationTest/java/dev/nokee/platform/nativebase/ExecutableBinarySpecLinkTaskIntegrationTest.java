...
class ExecutableBinarySpecLinkTaskIntegrationTest {
	...
	void createSubject() {
		...
		val variantIdentifier = VariantIdentifier.of("wuri", Variant.class, componentIdentifier);
	...
	void usesBinaryNameAsTaskNameVariant() {
		assertThat(subject, named("linkMedeWuriRuro"));
	}
	...
	@Test
	void hasDescription() {
		assertThat(subject, TaskMatchers.description("Links the binary ':mede:wuri:ruro'."));
	}
	...
	@Test
	void hasDestinationDirectoryUnderExesInsideBuildDirectory() {
		assertThat(subject.getDestinationDirectory(),
			providerOf(aFile(withAbsolutePath(containsString("/build/exes/")))));
	}
	...
	void includesBinaryNameInDestinationDirectory() {
		assertThat(subject.getDestinationDirectory(), providerOf(aFileNamed("ruro")));
	}
