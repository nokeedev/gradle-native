...
class StaticLibraryBinarySpecToStringIntegrationTest {
	...
	void createSubject() {
		...
		subject = registry.register(factory.create(projectIdentifier.child("wuca"))).as(StaticLibraryBinary.class).get();
	}

	@Test
	void checkToString() {
		assertThat(subject, hasToString("static library binary 'wuca'"));
	}
}
