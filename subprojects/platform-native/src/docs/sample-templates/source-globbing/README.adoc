...
= {c-application} Custom Source Layout Globbing
:summary: Set custom source layout Globbing for a {c-application}.
:type: sample-chapter
:tags: sample, {application}, sources, native, {C}, gradle
:category: {C}
:description: See how to set the source layout globbing of {an-application} implemented in {C} using the Gradle Nokee plugins.
